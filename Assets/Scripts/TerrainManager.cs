...
public class TerrainInfo
{
    ...
    public int get_i_index(float x, int co = 1)
    {
        int index = (int) Mathf.Floor(x_N * co * (x - x_low) / (x_high - x_low));
        if (index < 0)
        {
            index = 0;
        }else if (index > x_N * co - 1)
        {
            index = x_N * co - 1;
        }
        return index;
    ...
    public int get_j_index(float z, int co = 1) // get index of given coordinate
    {
        int index = (int)Mathf.Floor(z_N * co * (z - z_low) / (z_high - z_low));
        if (index < 0)
        {
            index = 0;
        }
        else if (index > z_N * co - 1)
        {
            index = z_N * co - 1;
        }
        return index;
    ...
    public float get_x_pos(int i, int co = 1)
    {
        float step = (x_high - x_low) / x_N / co;
        return x_low + step / 2 + step * i;
    }

    public float get_z_pos(int j, int co = 1) // get position of given index
    {
        float step = (z_high - z_low) / z_N / co;
